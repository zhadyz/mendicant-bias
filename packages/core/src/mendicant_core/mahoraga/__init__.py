...
class MahoragaEngine:
    ...
    automatically on matching contexts.

    Uses embedding-based semantic matching when sentence-transformers is
    available, falls back to keyword-overlap Jaccard similarity otherwise.
    """
    ...
    def __init__(
        ...
        self._rule_embeddings: dict[str, list[float]] = {}  # rule_id -> embedding
        self._load()
    ...
    def _load_encoder(self) -> Any:
        ...
        try:
            ...
            logger.debug("[Mahoraga] Loaded encoder: %s", self._embedding_model)
        except ImportError:
            self._encoder = None
            logger.debug("[Mahoraga] sentence-transformers not available; using keyword matching")
        return self._encoder

    def _get_rule_embedding(self, rule: AdaptationRule) -> list[float] | None:
        """Get or compute embedding for a rule's trigger + action text."""
        if rule.id in self._rule_embeddings:
            return self._rule_embeddings[rule.id]

        encoder = self._load_encoder()
        if encoder is None:
            return None

        # Combine trigger and action for richer semantic representation
        text = f"{rule.trigger} {rule.action}"
        vec = encoder.encode(text, convert_to_numpy=True).tolist()
        self._rule_embeddings[rule.id] = vec
        return vec
    ...
    def get_applicable_rules(
        ...
        is available, falls back to keyword-overlap Jaccard similarity.
        Returns rules sorted by relevance * confidence (highest first).
        """
        if not context:
            return []
        ...
        if encoder is not None:
            context_vec = encoder.encode(context, convert_to_numpy=True).tolist()
        ...
        for rule in self._rules:
            ...
            # Primary: embedding similarity (if available)
            emb_score = 0.0
            if context_vec is not None:
                rule_vec = self._get_rule_embedding(rule)
                if rule_vec is not None:
                    emb_score = _cosine_similarity(context_vec, rule_vec)
            ...
            keyword_score = (trigger_score * 0.5) + (tag_overlap * 0.3) + (action_score * 0.2)

            # Blend: prefer embeddings when available, keywords as boost
            if emb_score > 0:
                relevance = (emb_score * 0.7) + (keyword_score * 0.3)
            else:
                relevance = keyword_score

            # Threshold: include if there's meaningful relevance
            if relevance > 0.05 or emb_score > 0.3 or trigger_score > 0.15:
                rank = relevance * rule.confidence
                candidates.append((rank, rule))

        candidates.sort(key=lambda x: x[0], reverse=True)

        # Attach ranking score to each rule for downstream filtering
        results = []
        for rank, rule in candidates:
            rule._relevance_score = rank  # type: ignore[attr-defined]
            results.append(rule)
        return results
