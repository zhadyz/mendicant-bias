...
class MendicantConfig(BaseModel):
    ...
    def build_all_middleware(self) -> list:
        """Build all configured middleware in correct execution order.

        Order follows the middleware chain convention:
        1. SmartTaskRouter (before_agent) — classify task, set runtime flags
        2. SemanticToolRouter (before_agent) — select relevant tools
        3. ContextBudget (before_model) — enforce token budget
        4. LoopDetection (after_model) — detect repetitive tool calls
        5. ToolErrorHandling (wrap_tool_call) — graceful error recovery
        6. Verification (after_agent) — blind quality gate (if enabled)
        7. AdaptiveLearning (after_agent) — record patterns
        """
        from mendicant_core.middleware.loop_detection import LoopDetectionMiddleware
        from mendicant_core.middleware.tool_error_handling import ToolErrorHandlingMiddleware

        middlewares = [
            self.build_smart_task_router_middleware(),
            self.build_semantic_tool_router_middleware(),
            self.build_context_budget_middleware(),
            LoopDetectionMiddleware(),
            ToolErrorHandlingMiddleware(),
        ]
        if self.verification.enabled:
            middlewares.append(self.build_verification_middleware())
        middlewares.append(self.build_adaptive_learning_middleware())
        return middlewares
