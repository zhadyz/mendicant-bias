...
def mcp(ctx: click.Context) -> None:
    ...
    try:
        from mendicant_mcp.server import main as run_mcp  # type: ignore[import-untyped]
    except ImportError:
        console.print(
        ...
        "SessionStart": [
            {
                "hooks": [
                    {
                        "type": "command",
                        "command": "python -m mendicant_bias.hook_handler",
                        "timeout": 10,
                    }
                ]
            }
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Also configure MCP server for explicit tool access",
)
def install(target_dir: str, with_mcp: bool) -> None:
    """One-command setup: init + hooks + optional MCP. Ready to use."""
    _print_banner()
    base = Path(target_dir).resolve()

    # --- Step 1: Create .mendicant/ ---
    mendicant_dir = base / ".mendicant"
    mendicant_dir.mkdir(parents=True, exist_ok=True)
    ...
    for name, default in [
        ...
        f = mendicant_dir / name
        if not f.exists():
            f.write_text(default)

    console.print(f"[green]Created[/green] {mendicant_dir}")

    # --- Step 2: Write hooks to .claude/settings.local.json ---
    ...
    # Merge hooks — don't overwrite other settings
    existing["hooks"] = HOOKS_CONFIG["hooks"]
    settings_file.write_text(
        json.dumps(existing, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    console.print(f"[green]Configured hooks[/green] in {settings_file}")

    # --- Step 3: Optional MCP config ---
    if with_mcp:
        mcp_file = claude_dir / "mcp.json" if not (base / ".mcp.json").exists() else base / ".mcp.json"
        # Use project-level .mcp.json
        ...
        mcp_data["mcpServers"]["mendicant-bias"] = {
            "command": "mendicant",
            "args": ["mcp"],
        }
        mcp_file.write_text(
        ...
        console.print(f"[green]Configured MCP[/green] in {mcp_file}")

    # --- Summary ---
    console.print()

    table = Table(
        title="Mendicant Bias — Installed",
        show_header=True,
        header_style="bold cyan",
        border_style="dim",
    )
    ...
    table.add_row(".mendicant/", "config + data files")
    table.add_row("SessionStart hook", "loads adaptations + memory")
    table.add_row("UserPromptSubmit hook", "classifies + injects rules")
    table.add_row("PreToolUse hook", "code guidance on Write/Edit")
    table.add_row("PostToolUse hook", "verification + pattern recording")
    if with_mcp:
        table.add_row("MCP server", "20+ tools available")

    console.print(table)
    console.print()
    console.print("[dim]Restart Claude Code in this project. Mendicant is active.[/dim]")
