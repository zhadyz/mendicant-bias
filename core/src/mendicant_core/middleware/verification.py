...
class VerificationMiddleware(AgentMiddleware[VerificationState]):
    ...
    def _get_llm(self) -> Any:
        ...
        construction time.  Otherwise falls back to importing
        ``langchain_openai.ChatOpenAI`` directly.
        """
        if self._model_factory is not None:
            return self._model_factory()

        # Fallback: try langchain_openai directly
        try:
            ...
            raise RuntimeError(
                "Could not import langchain_openai.ChatOpenAI. Either install "
                "langchain-openai or provide a model_factory callable."
            ) from exc
