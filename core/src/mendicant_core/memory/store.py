...
class MemoryStore:
    ...
    def search_facts(self, query: str, limit: int = 10) -> list[Fact]:
        """Simple keyword search over facts.

        Returns facts whose content contains any of the query words,
        sorted by number of matching words (descending), then confidence.
        """
        data = self.load()
        if not query.strip():
            # Return all facts sorted by confidence descending
        ...
        for fact in data.facts:
            ...
            matches = sum(1 for w in query_words if w in fact_lower)
            if matches > 0:
                scored.append((matches, fact.confidence, fact))
